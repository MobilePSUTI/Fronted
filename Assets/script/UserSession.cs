...
public static class UserSession
{
    ...
    public static Texture2D CachedAvatar { get; set; }
    ...
    public static void SaveSession(bool rememberMe)
    {
        if (rememberMe && CurrentUser != null)
        {
            PlayerPrefs.SetInt("RememberMe", 1);
            PlayerPrefs.SetString("AuthToken", CurrentUser.token);
    ...
    public static bool HasSavedSession()
    {
        return PlayerPrefs.GetInt("RememberMe", 0) == 1 &&
               !string.IsNullOrEmpty(PlayerPrefs.GetString("AuthToken", ""));
    }
}